import openai



def format_input_text(text):
    prompt = """"
            You are a helpful assistant. Your task is to determine the sentiment of a given text. The sentiment can be either positive, negative, or neutral.
            You shall also provide a very brief summary of the text in swedish. Here is the text:
            {}
            """.format(text)
    return prompt


def split_sentiment_and_summary(text):
    split = [tmp for tmp in text.split("\n") if tmp != ""]
    out_dict = {}
    for item in split:
        splitted_txt = item.split(":")
        out_dict[splitted_txt[0]] = splitted_txt[1]
    
    return out_dict


def inference(prompt):
    client = openai.OpenAI(
    base_url="https://api.llm7.io/v1",
    api_key="unused"  # No key needed
    )


    response = client.chat.completions.create(
        model="gpt-4o-mini-2024-07-18",
        messages=[
            {"role": "user", "content": prompt}
        ]
    )
    return response


prompt = format_input_text("Idag har jag inte gjort något särskillt")
response = inference(prompt)
out_dict =split_sentiment_and_summary(response.choices[0].message.content)
print(out_dict)